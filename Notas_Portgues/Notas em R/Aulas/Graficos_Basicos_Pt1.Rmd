---
title: "Lucas Caramês - Gráficos Básicos"
output: graficos_basicos
---
```{r}
trees
```

Histogramas
```{r}
hist(trees$Height)
#                 definindo título #eixo vertical    eixo horizontal  cor das barras
hist(trees$Height, main="Árvores", ylab="Frequência", xlab="Altura", col="green")


hist(trees$Height, main="Árvores", ylab="Frequência", xlab="Altura", col="blue", density=20, breaks=20)
```
Densidade
```{r}
densidade = density(trees$Height)
plot(densidade)
```


Densidade sobre o Histograma, parâmetro par
```{r}
hist(trees$Height, main=NULL, xlab=NULL, ylab=NULL, col="grey")
par(new=TRUE)
plot(densidade)
```



Dispersão
```{r}
plot(trees$Girth, trees$Volume)
plot(trees$Girth, trees$Volume, main="Árvores")
plot(trees$Girth, trees$Volume, ylab="Circunferência", xlab="Volume", col="blue", main="Árvores")
#pch muda o elemento gráfico
plot(trees$Girth, trees$Volume, ylab="Circunferência", xlab="Volume", col="blue", main="Árvores", pch=20)
```



Muda tipo
```{r}
plot(trees$Girth, trees$Volume, ylab="Circunferência", xlab="Volume", col="blue", main="Árvores", pch=20, type="l")
```

Tremulação, diminui sobre posição
```{r}
plot(jitter(trees$Girth), trees$Volume, ylab="Circunferência", xlab="Volume", col="blue", main="Árvores")
```


Legenda com dimensão categórica
```{r}
CO2
plot(CO2$conc, CO2$uptake, pch=20, col=CO2$Treatment)
# "bottom", "bottomleft", "left", "topleft", "top", "topright", "right" and "center"
legend("bottomright", legend=c("nonchilled", "chilled"), cex=1, fill=c("black", "red"))
```


Novos dados
```{r}
plot(trees)
```


Divisão de Tella
```{r}
#Executar com ctrl+shift+enter
split.screen((fig=c(2,2)))
screen(1)
plot(trees$Girth, trees$Volume)
screen(2)
plot(trees$Girth, trees$Height)
screen(3)
plot(trees$Height, trees$Volume)
screen(4)
hist(trees$Volume)
close.screen(all=TRUE)
```